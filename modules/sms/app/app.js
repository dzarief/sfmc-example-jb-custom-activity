// JOURNEY BUILDER CUSTOM ACTIVITY - sms ACTIVITY
// ````````````````````````````````````````````````````````````
...
// setup the sms example app
module.exports = function discountCodeExample(app, options) {
    const moduleDirectory = `${options.rootDirectory}/modules/sms`;

    // setup static resources
    app.use('/modules/sms/dist', express.static(`${moduleDirectory}/dist`));
    app.use('/modules/sms/images', express.static(`${moduleDirectory}/images`));

    // setup the index redirect
    app.get('/modules/sms/', function(req, res) {
        return res.redirect('/modules/sms/index.html');
    });

    // setup index.html route
    app.get('/modules/sms/index.html', function(req, res) {
    ...
    app.get('/modules/sms/config.json', function(req, res) {
     ...
     */
    app.post('/modules/sms/save', function(req, res) {
        console.log('debug: /modules/sms/save');
     ...
     * 50x - Return if the configuration is invalid (this will block the publish phase)
     */
    app.post('/modules/sms/publish', function(req, res) {
        console.log('debug: /modules/sms/publish');
        return res.status(200).json({});
    });
     ...
     * 50x - Return if the configuration is invalid (this will block the publish phase)
     */
    app.post('/modules/sms/validate', function(req, res) {
        console.log('debug: /modules/sms/validate');
        return res.status(200).json({});
    });
     ...
     * @return {[type]}
     */
    app.post('/modules/sms/stop', function(req, res) {
        console.log('debug: /modules/sms/stop');
        return res.status(200).json({});
    });
     ...
     */
    app.post('/modules/sms/execute', function(req, res) {
        console.log('debug: /modules/sms/execute');
